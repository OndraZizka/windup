...
public class GraphContextImpl implements GraphContext
{
    ...
    public void init(GraphContextConfig config){
        if(this.eventGraph != null){
            final String msg = "Graph was already initialized, see cause's stacktrace for where.";
            if(this.config != null && this.config.isThrowOnLazyInit())
                throw new IllegalStateException(msg, stripProxyCalls(this.initStack));
            else
                log.log(Level.WARNING, msg, stripProxyCalls(this.initStack));
        }
